...
poly_order=1;  %best s-w but worse peak and cumalative% change to 1
...
%entropy
Entropy_unalign =-(sum(sum_unalign.*log(sum_unalign)))/601
...
hold on
plot(ShiftsRequiredArray(:,1),s_l_1)
...
%entropy
Entropy_peak =-(sum(sum_peak.*log(sum_peak)))/601
...
figure; imagesc(Range_axis, NumProfiles_axis, 20*log10(abs(Aligned_Profiles_new_normalised3))); colorbar
xlabel('Range (m)');
ylabel('Profile number');
...
%contrast and entropy function
sum_avg=sum(abs(Aligned_Profiles_new_normalised3))/256;
Contrast_avg = sum(sum_avg.^2,2)/601
%entropy
Entropy_avg =-(sum(sum_avg.*log(sum_avg)))/601


%%
%%% Align HRR profiles using envelope correlation (Sliding Window avg reference profile :moving avg) %%%

w= 42; %window_length or 44 for min ent      was 42
...
figure; imagesc(Range_axis, NumProfiles_axis, 20*log10(abs(Aligned_Profiles_new_normalised4))); colorbar
xlabel('Range (m)');
ylabel('Profile number');
...
%contrast and entropy function
sum_sw_avg=sum(abs(Aligned_Profiles_new_normalised4))/256;
