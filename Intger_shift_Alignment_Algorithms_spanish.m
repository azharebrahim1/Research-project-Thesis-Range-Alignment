...
HRR_profiles = Dataset.HRR_profiles(x:y,:);
%HRR_profiles = (HRR_profiles)/max(max(abs(HRR_profiles)));   %normalised
ProfileRepetitionFreq = Dataset.ProfileRepitionFreq;
Range_axis = Dataset.Range_axis; 

HRR_profiles_normalised = (HRR_profiles)/max(max(abs(HRR_profiles)));   %normalised
% Plot HRR profiles
NumProfiles_axis = 1:size(HRR_profiles_normalised,1); 
figure; imagesc(Range_axis, NumProfiles_axis, 20*log10(abs(HRR_profiles_normalised))); colorbar
xlabel('Range (m)');
...
%contrast function
sum_unalign=sum(abs(HRR_profiles_normalised))/256;
Contrast_unalign = sum(sum_unalign.^2,2)/601
%entropy
Entropy_unalign =-(sum(sum_unalign.*log(sum_unalign),2))/601

%{
%comparison C and H
sum_unalign_comparison=sum(abs(HRR_profiles_normalised))/256;
Contrast_unalign_comparison = sum(sum_unalign_comparison.^2,2)/601
Contrast_unalign_db_comparison=10*log10(Contrast_unalign_comparison)
Entropy_unalign_comparison =-(sum(sum_unalign_comparison.*log(sum_unalign_comparison),2))/601
...
for i=1:(length(HRR_profiles(:,1)))
    ...
    [a b] = xcorr( abs(HRR_profiles(1,:)), abs(HRR_profiles(i,:)) ); %choose reference profile e.g. 3
    [maxval maxidx] = max(a);
    ShiftReq = b(maxidx);
...
figure; imagesc(Range_axis, NumProfiles_axis, 20*log10(abs(Aligned_Profiles_normalised))); colorbar
xlabel('Range (m)');
ylabel('Profile number');
...
ylabel("Integer shifts required")
title('Number of shifts required per profile: SRF')

%contrast and entropy function
sum_peak=sum(abs(Aligned_Profiles_normalised))/256;
Contrast_peak = sum(sum_peak.^2,2)/601
%entropy
Entropy_peak =-(sum(sum_peak.*log(sum_peak),2))/601
...
figure; imagesc(Range_axis, NumProfiles_axis, 20*log10(abs(Aligned_Profiles_normalised2))); colorbar
xlabel('Range (m)');
ylabel('Profile number');
...
%contrast and entropy function
sum_adj=sum(abs(Aligned_Profiles_normalised2))/256;
Contrast_adj = sum(sum_adj.^2,2)/601
%entropy
Entropy_adj =-(sum(sum_adj.*log(sum_adj)))/601
...
figure; imagesc(Range_axis, NumProfiles_axis, 20*log10(abs(Aligned_Profiles_normalised3))); colorbar
xlabel('Range (m)');
ylabel('Profile number');
...
%contrast and entropy function
sum_avg=sum(abs(Aligned_Profiles_normalised3))/256;
Contrast_avg = sum(sum_avg.^2,2)/601
%entropy
Entropy_avg =-(sum(sum_avg.*log(sum_avg),2))/601


%%
%%% Align HRR profiles using envelope correlation (Sliding Window avg reference profile :moving avg) %%%

n= 42; %window_length or 44 for min ent
...
figure; imagesc(Range_axis, NumProfiles_axis, 20*log10(abs(Aligned_Profiles_normalised4))); colorbar
xlabel('Range (m)');
ylabel('Profile number');
...
%contrast and entropy function
sum_sw_avg=sum(abs(Aligned_Profiles_normalised4))/256;
Contrast_sw_avg = sum(sum_sw_avg.^2,2)/601
%entropy
Entropy_sw_avg =-(sum(sum_sw_avg.*log(sum_sw_avg)))/601
...
array_combined=[normalize(array_con2,'range') + normalize(array_ent2,'range')];    %normalised data
...
Entropy_sw_avg_db =10*log10(Entropy_sw_avg)  % i removed - before entropy
